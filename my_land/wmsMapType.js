...
function WmsMapType(name, url, params, options) {
	...
	this.getTile = function(coord, zoom, ownerDocument) {
		...
		if(this.params['SR'] != undefined){
			this.params['srs'] = this.params['SR'];
			proj4.defs([
				  [
				    "EPSG:6706",
				    '+title=RDN2008 +proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs +units=degrees ']
				]);

			var firstProjection =  this.params['SR'] ;
			//var secondProjection = "+datum=" + this.params['SR'];
			var sw = proj4("GOOGLE",firstProjection,[bounds.swX,bounds.swY]);
			var ne = proj4("GOOGLE",firstProjection,[bounds.neX,bounds.neY]);
			bounds = {
				swX : sw[0],
				swY : sw[1],
				neX : ne[0],
